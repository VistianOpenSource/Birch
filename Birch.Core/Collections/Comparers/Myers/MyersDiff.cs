   ...
   /// <summary>
    ...
    public sealed class MyersDiff<T>
    {
        ...
        public EditScript Execute()
        {
            ...
            var subArrayA = new SubArray<T>(_listA);
            var subArrayB = new SubArray<T>(_listB);
            ...
            var result = ConvertMatchPointsToEditScript(subArrayA.Length, subArrayB.Length, matchPoints);
        ...
        /// </summary>
        /// <returns>Reverse SES length</returns>
        public int GetReverseShortestEditScriptLength()
        {
            ...
            if (SetupFictitiousPoints(subArrayA, subArrayB))
            {
                ...
                for (var d = 0; d <= n + m; d++)
                {
                    for (var k = -d; k <= d; k += 2)
                    {
                        var x = GetReverseDPaths(subArrayA, subArrayB, d, k, delta);
                        var y = x - (k + delta);
                        if (x <= 0 && y <= 0)
                        {
                            return d;
        ...
        {
            var result = GetSimilarity(_listA.Count, _listB.Count, GetLongestCommonSubsequenceLength());
            return result;
        }
        ...
        private int FindMiddleSnake(SubArray<T> subArrayA, SubArray<T> subArrayB, out int pathStartX, out int pathEndX, out int pathK)
        {
            ...
            var delta = subArrayA.Length - subArrayB.Length;
            var ceiling = (int)Math.Ceiling((subArrayA.Length + subArrayB.Length) / 2.0);
                ...
                for (var k = -d; k <= d; k += 2)
                {
                    ...
                    if (delta % 2 != 0 && k >= delta - (d - 1) && k <= delta + (d - 1))
                    {
                        ...
                        if (_vectorForward[k] >= _vectorReverse[k])
                        {
                            ...
                            while (pathStartX > 0 && pathStartY > 0 && _equalityComparer.Equals(subArrayA[pathStartX],subArrayB[pathStartY]) )
                            {
                                pathStartX--;
                                pathStartY--;
                            }
                    ...
                    if (delta % 2 == 0 && k + delta >= -d && k + delta <= d)
                    {
                        ...
                        if (_vectorReverse[k + delta] <= _vectorForward[k + delta])
                        {
                            ...
                            while (pathEndX < subArrayA.Length &&
                                   ...
                                   _equalityComparer.Equals(subArrayA[pathEndX + 1],subArrayB[pathEndY + 1]))
                            {
                                pathEndX++;
                                pathEndY++;
                            }
        ...
        private int GetForwardDPaths(SubArray<T> subArrayA, SubArray<T> subArrayB, int d, int k)
        {
            ...
            while (x < subArrayA.Length && y < subArrayB.Length && _equalityComparer.Equals(subArrayA[x + 1],subArrayB[y + 1]))
            {
                x++;
                y++;
            }
        ...
        private void GetLcs(SubArray<T> subArrayA, SubArray<T> subArrayB, List<T> output)
        {
            while (true)
            {
                if (subArrayA.Length > 0 && subArrayB.Length > 0)
                {
                    ...
                    {
                        for (var i = 1; i <= subArrayA.Length; i++)
                        {
                            output.Add(subArrayA[i]);
                        }
                    }
                    ...
                    {
                        for (var i = 1; i <= subArrayB.Length; i++)
                        {
        ...
        private void GetMatchPoints(SubArray<T> subArrayA, SubArray<T> subArrayB, ICollection<Point2D<int>> matchPoints)
        {
            while (true)
            {
                if (subArrayA.Length > 0 && subArrayB.Length > 0)
                {
                    ...
                    var n = subArrayA.Length;
                    var m = subArrayB.Length;
                    if (m > n)
                    {
                        ...
                        for (var i = 1; i <= n; i++)
                        {
                            ...
                            if (!_equalityComparer.Equals(subArrayA[i],subArrayB[currentY]))
                            {
                                currentY++;
                            }
                    ...
                    else
                    {
                        ...
                        {
                            // We must skip the one difference when we hit it
                            if (!_equalityComparer.Equals(subArrayA[currentX],subArrayB[i]))
                            {
                                currentX++;
                            }
        ...
        private int GetReverseDPaths(SubArray<T> subArrayA, SubArray<T> subArrayB, int d, int k, int delta)
        {
            ...
            while (x > 0 && y > 0 && _equalityComparer.Equals(subArrayA[x],subArrayB[y]) )
            {
                x--;
                y--;
            }
